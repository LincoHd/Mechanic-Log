...
void AppChart::draw(Tracker* tracker, const char* title, bool* p_open, ImGuiWindowFlags flags)
{
    ImGui::SetNextWindowSize(ImVec2(500,400), ImGuiCond_FirstUseEver);
    ImGui::Begin(title, p_open, flags);
...
{
	writeToDisk(tracker);
	//ShellExecuteA(nullptr, "open", last_file_path.c_str(), nullptr, (!Settings::export_dir.empty() ? Settings::export_dir.c_str() : AddonPath.string().c_str()), SW_HIDE); //TODO: Maybe open directly 
}
...
void AppChart::writeToDisk(Tracker* tracker)
{
	...
	std::string file_path = (!Settings::export_dir.empty() ? Settings::export_dir : AddonPath.string())  + "\\" + std::string(time_str) + "-" + std::to_string(new_export_total) + ".csv";
...
void AppOptions::draw(Tracker* tracker)
{
	if (ImGui::BeginChild("Mechanics Settings", ImVec2(550, 650)))
	{
		...
		ImGui::Text("Show Mechanic Chart");
		if (ImGui::Checkbox("Enabled##Chart", &Settings::show_app_chart))
		{
